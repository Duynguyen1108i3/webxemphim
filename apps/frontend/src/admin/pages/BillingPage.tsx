  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-white/10">
        <div>
          <h2 className="text-2xl md:text-3xl font-black hero-title text-white">Quản Lý Doanh Thu & Giao Dịch</h2>
          <p className="text-xs md:text-sm text-white/50 mt-0.5">
            Thống kê thanh toán và đối soát giao dịch qua các cổng thanh toán
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={() => refetch()}
            className="h-10 w-10 inline-flex items-center justify-center rounded-full glass-button text-white transition active:scale-95 cursor-pointer"
            title="Làm mới"
          >
            <RefreshCw size={15} />
          </button>
          <button
            type="button"
            onClick={() => showToast("Đã xuất báo cáo doanh thu thành công!")}
            className="h-10 px-5 inline-flex items-center justify-center gap-2 rounded-full bg-white hover:bg-white/90 active:bg-white/80 text-xs font-bold text-black transition shadow-xl cursor-pointer"
          >
            <Download size={14} />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="liquid-glass-card rounded-2xl p-5 shadow-xl">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-white/60">Tổng Doanh Thu</span>
          ...
          <p className="mt-1 text-[11px] text-emerald-400 font-semibold">
            {(data?.summary?.totalVolumeVND ?? 0) === 0 ? "Chưa có giao dịch phát sinh" : "Giao dịch đã xác nhận"}
          </p>
        </div>
        ...
        <div className="liquid-glass-card rounded-2xl p-5 shadow-xl">
          ...
          <p className="mt-3 text-2xl font-black font-spartan text-white">
            {(data?.summary?.vietqrShare ?? 0)}% Tỷ trọng
          </p>
          <p className="mt-1 text-[11px] text-white/40 font-medium">
            {(data?.summary?.totalVolumeVND ?? 0) === 0 ? "Sẵn sàng nhận thanh toán QR" : "Thanh toán qua mã QR"}
          </p>
        </div>

        <div className="liquid-glass-card rounded-2xl p-5 shadow-xl">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-white/60">Ví MoMo</span>
            <span className="p-2 rounded-xl bg-pink-500/10 text-pink-400 border border-pink-500/20">
              <Smartphone size={16} />
            </span>
          </div>
          <p className="mt-3 text-2xl font-black font-spartan text-white">
            {(data?.summary?.momoShare ?? 0)}% Tỷ trọng
          </p>
          <p className="mt-1 text-[11px] text-white/40 font-medium">
            {(data?.summary?.totalVolumeVND ?? 0) === 0 ? "Sẵn sàng nhận ví MoMo" : "Thanh toán qua ví MoMo"}
          </p>
        </div>
